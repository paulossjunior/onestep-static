...
**Total de projetos:** {{ group_projects|length }}
...
{% set group_projects = group_projects[['Título', 'Período', 'Coordenador', 'Pesquisadores', 'Estudantes', 'Linha de Pesquisa', 'Natureza', 'Palavras-chave', 'Parceiro', 'Pub/Fin']] %}
...
(function() {
  ...
  var layout = {
    title: 'Número de Estudantes ao Longo do Tempo',
    xaxis: {
      title: 'Ano',
      dtick: 1,
      tickmode: 'linear'
...
**Total de estudantes únicos:** {{ student_projects|length }}
...
**Este grafo de rede visualiza as relações de colaboração entre todas as pessoas envolvidas nos projetos do grupo de pesquisa.**

<div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; margin: 20px 0;">
  <h4 style="margin-top: 0;">Como Ler Este Gráfico:</h4>
  <ul style="margin-bottom: 0;">
    <li><strong>Nós (círculos)</strong> representam pessoas: coordenadores, pesquisadores e estudantes</li>
    <li><strong>Tamanho do nó</strong> indica quantas conexões uma pessoa tem (maior = mais colaborativo)</li>
    <li><strong>Arestas (linhas)</strong> conectam pessoas que trabalharam juntas em projetos</li>
    <li><strong>Espessura da aresta</strong> mostra força da colaboração (mais espessa = mais projetos juntos)</li>
    <li><strong>Cores</strong>: Vermelho = Coordenadores, Turquesa = Pesquisadores, Verde Claro = Estudantes</li>
    <li><strong>Interativo</strong>: Passe o mouse sobre nós/arestas para detalhes, arraste para reorganizar, role para zoom</li>
  </ul>
</div>
...
(function() {
  ...
  legend.innerHTML = '<strong style="font-size:14px;">Legend</strong><br><br>' +
    '<div style="margin:5px 0;"><span style="color:#FF6B6B;font-size:20px;">●</span> <strong>Coordenadores</strong></div>' +
    '<div style="margin:5px 0;"><span style="color:#4ECDC4;font-size:20px;">●</span> <strong>Pesquisadores</strong></div>' +
    '<div style="margin:5px 0;"><span style="color:#95E1D3;font-size:20px;">●</span> <strong>Estudantes</strong></div>' +
    '<hr style="margin:10px 0;">' +
    '<small><strong>Tamanho do nó:</strong> # conexões<br><strong>Espessura da aresta:</strong> # colaborações</small>';
...
<div style="background-color: #e7f5ff; padding: 20px; border-radius: 8px; margin: 20px 0;">
  <h4 style="margin-top: 0; color: #1971c2;">Estatísticas da Rede</h4>
  <table style="width:100%; border-collapse: collapse;">
    <tr>
      <td style="padding:5px;"><strong>Total de Pessoas:</strong></td>
      <td>{{ stats.get('total_people', 0) }}</td>
      <td style="padding:5px;"><strong>Total de Conexões:</strong></td>
      <td>{{ stats.get('total_conexões', 0) }}</td>
    </tr>
    <tr>
      <td style="padding:5px;"><strong>Coordenadores:</strong></td>
      <td>{{ stats.get('coordinators', 0) }}</td>
      <td style="padding:5px;"><strong>Pesquisadores:</strong></td>
      <td>{{ stats.get('researchers', 0) }}</td>
    </tr>
    <tr>
      <td style="padding:5px;"><strong>Estudantes:</strong></td>
      <td>{{ stats.get('students', 0) }}</td>
      <td style="padding:5px;"><strong>Média de Conexões/Pessoa:</strong></td>
      <td>{{ stats.get('avg_conexões_per_person', 0) }}</td>
    </tr>
    {% if stats.get('most_connected_person') %}
    <tr>
      <td colspan="4" style="padding:5px;">
        <strong>Pessoa Mais Conectada:</strong> {{ stats.most_connected_person.get('name', '') }} 
        ({{ stats.most_connected_person.get('conexões', 0) }} conexões)
      </td>
    </tr>
    {% endif %}
    ...
    <tr>
      <td colspan="4" style="padding:5px;">
        <strong>Colaboração Mais Forte:</strong> {{ stats.strongest_collaboration.get('people', [])|join(' ↔ ') }} 
        ({{ stats.strongest_collaboration.get('projects_together', 0) }} projetos juntos)
      </td>
    </tr>
    {% endif %}
  </table>
