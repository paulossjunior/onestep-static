...
{# Display statistics #}
**Total de Projetos:** {{ df|length }}  
**Anos:** {{ unique_years|length }} ({{ unique_years[-1] if unique_years else 'N/A' }} - {{ unique_years[0] if unique_years else 'N/A' }})

---

## Visualizações

Esta seção apresenta análises visuais de projetos de pesquisa ao longo do tempo, fornecendo insights sobre tendências de projetos, envolvimento de estudantes e padrões de participação.

### Projetos ao Longo do Tempo

O gráfico a seguir mostra a evolução dos projetos de pesquisa em categorias mutuamente exclusivas. A linha **Todos os Projetos** (em negrito) representa o total. Os projetos são categorizados como: **Financiamento + Parceiros** (ambos), **Apenas Financiamento** (sem parceiros), **Apenas Parceiros** (sem financiamento) ou **Nenhum** (projetos independentes). A soma das quatro categorias é igual ao total.
...
(function() {
  ...
  var layout = {
    title: 'Projetos ao Longo do Tempo por Categoria',
    xaxis: {
      title: 'Ano',
      dtick: 1,
      tickmode: 'linear'
...
Este gráfico mostra a distribuição de projetos por status de conclusão. A linha **Concluídos** representa projetos que foram finalizados, enquanto a linha **Em Andamento** mostra projetos que ainda estão em progresso. Isso ajuda a rastrear o equilíbrio entre pesquisas concluídas e investigações ativas.
...
### Projetos em Andamento - Conclusão Esperada por Ano

Este gráfico mostra quantos projetos em andamento devem ser concluídos a cada ano, ajudando a visualizar o pipeline de projetos e marcos futuros.
...
**Total de projetos em andamento:** {{ ongoing_projects_list|length }}
...
### Rede de Colaboração de Pesquisadores
...
### Projetos por Associação a Grupos de Pesquisa

Este gráfico mostra a distribuição de projetos associados a grupos de pesquisa versus projetos independentes ao longo do tempo, ajudando a entender o papel dos grupos de pesquisa na organização de projetos.
...
### Top 10 Linhas de Pesquisa

Este gráfico mostra as 10 linhas de pesquisa mais ativas com base no número de projetos, ajudando a identificar as principais áreas de foco de pesquisa no Campus Serra.
...
(function() {
  ...
  var layout = {
    title: 'Top 10 Linhas de Pesquisa por Número de Projetos',
    xaxis: {
...
<details style="margin: 20px 0;">
<summary style="cursor: pointer; font-weight: bold; padding: 10px; background-color: #e9ecef; border-radius: 5px;">
  📋 Ver Lista Completa de Linhas de Pesquisa ({{ research_line_count|length }})
</summary>
<div style="margin-top: 15px;">
<table style="width:100%; border-collapse: collapse;">
  <thead>
    <tr style="background-color: #e9ecef;">
      <th style="padding: 10px; text-align: left; border: 1px solid #dee2e6;">Linha de Pesquisa</th>
      <th style="padding: 10px; text-align: center; border: 1px solid #dee2e6;">Número de Projetos</th>
      <th style="padding: 10px; text-align: center; border: 1px solid #dee2e6;">Porcentagem</th>
    </tr>
  </thead>
  <tbody>
...
### Envolvimento de Estudantes ao Longo do Tempo

Este gráfico ilustra a participação de estudantes em projetos de pesquisa ao longo dos anos. A linha **Todos os Estudantes** mostra o número total de estudantes engajados anualmente, enquanto as linhas **Com Financiamento** e **Sem Financiamento** distinguem entre estudantes trabalhando em projetos financiados versus não financiados. Isso ajuda a identificar tendências em oportunidades de pesquisa para estudantes e o impacto do financiamento no envolvimento estudantil.
...
(function() {
  ...
  var layout = {
    ...
    xaxis: {
      title: 'Número de Projetos por Estudante',
      dtick: 1,
      tickmode: 'linear'
    },
    yaxis: {
...
<div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; margin: 20px 0;">
  <h4 style="margin-top: 0;">Estudantes Altamente Engajados (5+ Projetos)</h4>
  <p>Os seguintes estudantes demonstraram compromisso excepcional ao participar de 5 ou mais projetos de pesquisa:</p>
  <table style="width:100%; border-collapse: collapse;">
    <thead>
      <tr style="background-color: #e9ecef;">
        <th style="padding: 10px; text-align: left; border: 1px solid #dee2e6;">Nome do Estudante</th>
        <th style="padding: 10px; text-align: center; border: 1px solid #dee2e6;">Número de Projetos</th>
      </tr>
    </thead>
    <tbody>
  ...
  <p style="margin-bottom: 0; margin-top: 15px; font-size: 0.9em; color: #6c757d;">
    <strong>Total:</strong> {{ highly_engaged_students|length }} estudante(s) com envolvimento sustentado em pesquisa
  </p>
</div>
...
(function() {
  ...
  var layout = {
    title: 'Distribuição de Participação de Pesquisadores (Total: {{ researcher_project_count|length }} pesquisadores únicos)',
    xaxis: {
...
<div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; margin: 20px 0;">
  ...
  <p>Os seguintes pesquisadores demonstraram compromisso excepcional ao participar de 3 ou mais projetos de pesquisa:</p>
  <table style="width:100%; border-collapse: collapse;">
    <thead>
      <tr style="background-color: #e9ecef;">
...
Esta seção analisa as parcerias externas e colaborações estabelecidas através de projetos de pesquisa, destacando as principais organizações parceiras e grupos de pesquisa externos envolvidos.
...
<div style="background-color: #e7f5ff; padding: 20px; border-radius: 8px; margin: 20px 0;">
  <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 20px;">
    <div style="background-color: white; padding: 15px; border-radius: 5px; text-align: center;">
      ...
      <p style="margin: 5px 0; color: #666;">Projetos com Parceiros</p>
      <small style="color: #999;">{{ partnership_data['partners_percentage'] }}% do total</small>
    </div>
    <div style="background-color: white; padding: 15px; border-radius: 5px; text-align: center;">
      <h4 style="margin: 0; color: #2ca02c;">{{ partnership_data['unique_partners_count'] }}</h4>
      <p style="margin: 5px 0; color: #666;">Parceiros Únicos</p>
      <small style="color: #999;">Organizações</small>
    </div>
    <div style="background-color: white; padding: 15px; border-radius: 5px; text-align: center;">
      <h4 style="margin: 0; color: #d62728;">{{ partnership_data['projects_with_external_groups'] }}</h4>
      <p style="margin: 5px 0; color: #666;">Projetos com Grupos Externos</p>
      <small style="color: #999;">{{ partnership_data['external_groups_percentage'] }}% do total</small>
    </div>
    <div style="background-color: white; padding: 15px; border-radius: 5px; text-align: center;">
      <h4 style="margin: 0; color: #ff7f0e;">{{ partnership_data['unique_external_groups_count'] }}</h4>
      <p style="margin: 5px 0; color: #666;">Grupos Externos Únicos</p>
      <small style="color: #999;">Grupos de Pesquisa</small>
    </div>
  </div>
</div>
...
O gráfico a seguir mostra as 20 principais organizações parceiras que colaboraram em projetos de pesquisa, classificadas por número de projetos.
...
(function() {
  ...
  var layout = {
    title: 'Top 20 Organizações Parceiras',
    xaxis: {
      title: 'Número de Projetos',
      gridcolor: '#f0f0f0',
      showgrid: true
...
<details style="margin: 20px 0;">
<summary style="cursor: pointer; font-weight: bold; padding: 10px; background-color: #e9ecef; border-radius: 5px;">
  📋 Ver Lista Completa de Parceiros ({{ partnership_data['unique_partners_count'] }})
</summary>
<div style="margin-top: 15px;">
<table style="width:100%; border-collapse: collapse;">
  <thead>
    <tr style="background-color: #e9ecef;">
      <th style="padding: 10px; text-align: left; border: 1px solid #dee2e6;">Organização Parceira</th>
      <th style="padding: 10px; text-align: center; border: 1px solid #dee2e6;">Número de Projetos</th>
      <th style="padding: 10px; text-align: center; border: 1px solid #dee2e6;">Porcentagem</th>
    </tr>
  </thead>
...
#### Principais Grupos de Pesquisa Externos

Este gráfico exibe os principais grupos de pesquisa externos que colaboraram em projetos, mostrando a força das conexões de pesquisa interinstitucionais.
...
(function() {
  ...
  var layout = {
    title: 'Principais Grupos de Pesquisa Externos',
    xaxis: {
      title: 'Número de Projetos',
      gridcolor: '#f0f0f0',
      showgrid: true
...
<details style="margin: 20px 0;">
<summary style="cursor: pointer; font-weight: bold; padding: 10px; background-color: #e9ecef; border-radius: 5px;">
  📋 Ver Lista Completa de Grupos de Pesquisa Externos ({{ partnership_data['unique_external_groups_count'] }})
</summary>
<div style="margin-top: 15px;">
<table style="width:100%; border-collapse: collapse;">
  <thead>
    <tr style="background-color: #e9ecef;">
      <th style="padding: 10px; text-align: left; border: 1px solid #dee2e6;">Grupo de Pesquisa Externo</th>
      <th style="padding: 10px; text-align: center; border: 1px solid #dee2e6;">Número de Projetos</th>
      <th style="padding: 10px; text-align: center; border: 1px solid #dee2e6;">Porcentagem</th>
    </tr>
  </thead>
...
**Total de projetos:** {{ year_df|length }}
...
- **ID:** {{ row.id }}
- **Período:** {{ row.start_date }} to {{ row.end_date }}
- **Coordenador:** {{ row.coordinator }}{% if row.coordinator_email %} ([{{ row.coordinator_email }}](mailto:{{ row.coordinator_email }})){% endif %}

{% if row.research_group %}- **Grupo de Pesquisa:** {{ row.research_group }}
{% endif %}{% if row.knowledge_area %}- **Área de Conhecimento:** {{ row.knowledge_area }}
{% endif %}{% if row.research_line %}- **Linha de Pesquisa:** {{ row.research_line }}
{% endif %}{% if row.nature %}- **Natureza:** {{ row.nature }}
{% endif %}{% if row.researchers and row.researchers|length > 0 %}- **Pesquisadores:** {{ row.researchers|join(', ') }}
{% endif %}{% if row.students and row.students|length > 0 %}- **Estudantes:** {{ row.students|join(', ') }}
{% endif %}{% if row.keywords and row.keywords|length > 0 %}- **Palavras-chave:** {{ row.keywords|join(', ') }}
{% endif %}{% if row.partner %}- **Parceiro:** {{ row.partner }}
{% endif %}{% if row.publications_count != '0' or row.funding_count != '0' %}- **Publicações:** {{ row.publications_count }} | **Financiamento:** {{ row.funding_count }}
{% endif %}
{% endfor %}
